"use server";
import { eachMonthOfInterval, endOfYear, format, startOfMonth, startOfYear, subMonths } from "date-fns";
...
export async function validateInventory(
	...
	for (let i = 0; i < items.length; i++) {
		...
		if (inventory.quantity < inventory.threshold) {
			const warning = `Warning: Inventory for medication ${inventory.medication.name} is low. Current stock: ${
				inventory.quantity - item.quantity
			}, threshold: ${inventory.threshold}. Please consider restocking.`;
...
export async function updateInventoryLevels(items: AggregatedItem[], tenantId: string): Promise<UpdateInventoryItems[]> {
...
// Chart
export async function fetchInventoryLevels(): Promise<{ month: string; medication: string; quantity: number }[]> {
