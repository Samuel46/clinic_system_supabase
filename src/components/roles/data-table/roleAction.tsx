...
import { RoleDelete } from "../dialog";
import { deleteRoleAction } from "@actions/roles.action";

export default function RoleAction({ row }: { row: Row<Role> }) {
  ...
  const handleDelete = async () => {
    startTransition(() => {
      deleteRoleAction(row.original.id).then((result) => {
        if (result.success) {
