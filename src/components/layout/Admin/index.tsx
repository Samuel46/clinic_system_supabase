...
export function ApplicationLayout({ children, user, tenant }: Props) {
	...
	useEffect(() => {
		const channel = supabase
			...
			.on("postgres_changes", { event: "INSERT", schema: "public", table: "Patient" }, (payload) => {
				...
				if (tenant?.id === patient.tenantId && ["Admin", "Clinic"].includes(user?.role ?? "")) {
					toast.custom(
						(id) => (
							<div className="rounded-2xl bg-slate-50 p-4 w-full" key={id}>
								<div className="flex w-full">
									<div className="flex-shrink-0">
										<CheckCircleIcon aria-hidden="true" className="h-5 w-5 text-lime-400" />
									</div>
									<div className="ml-3">
										<h3 className="text-sm font-medium text-slate-800">{newMessage.title}</h3>
										...
										<div className="mt-4">
											<div className="-mx-2 -my-1.5 flex gap-x-2">
												...
												<Button size="sm" variant="ghost" onClick={() => toast.dismiss(id)} type="button">
													Dismiss
												</Button>
											</div>
										</div>
	...
	return (
		<SidebarLayout navbar={<NavbarAccount user={user} />} sidebar={<SideBarAccount user={user} tenant={tenant} />}>
			<ToasterProvider />
