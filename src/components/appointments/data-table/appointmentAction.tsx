...
import { AppointmentDelete } from "../dialog";
import { deleteAppointmentAction } from "@actions/appointments.action";
import { AppointmentDataTable } from "@type/index";

export default function AppointmentAction({ row }: { row: Row<AppointmentDataTable> }) {
  ...
  const handleEdit = async () => {
    push(`appointments/${row.original.id}`);
  };
  ...
  const handleDelete = async () => {
    startTransition(() => {
      deleteAppointmentAction(row.original.id).then((result) => {
        if (result.success) {
