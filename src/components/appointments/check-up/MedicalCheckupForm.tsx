...
import { createMedicalCheckupAction, updateMedicalCheckupAction } from "@actions/checkups.action";
import { zodResolver } from "@hookform/resolvers/zod";
import { Appointment, MedicalCheckup, MedicalRecord, Treatment } from "@prisma/client";
import { CreateMedicalCheckupInput, createMedicalCheckupSchema } from "@schemas/checkup.schemas";
...
export default function MedicalCheckupForm({ currentCheckup, user, appointment, record, treatment }: Props) {
	...
	const defaultValues = useMemo(
		() => ({
			...
			checkupDate: currentCheckup?.checkupDate ? new Date(currentCheckup.checkupDate) : new Date(),
		...
		<FadeIn className=" space-y-6 pt-10 grid ">
			...
			<FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
				...
				<div className="grid gap-6  ">
					...
					<Button disabled={isLoading} type="submit" variant="secondary" className=" p-6  font-bold  place-self-start">
			...
			{currentCheckup && (
				<Button
					type="submit"
					onClick={() => startTransition(() => router.push(`/admin/appointments/treatment?id=${appointment?.id}`))}
