...
import { deleteTreatmentAction } from "@actions/treatment.action";
import { TreatmentDelete } from "../dialog";

export default function TreatmentAction({ row }: { row: Row<TreatmentColumns> }) {
  ...
  const handleEdit = async () => {
    push(`/admin/treatments/create?id=${row.original.id}`);
  };

  const handleView = async () => {
    push(`treatments/${row.original.id}`);
  };
  ...
  const handleDelete = async () => {
    startTransition(() => {
      deleteTreatmentAction(row.original.id).then((result) => {
        if (result.success) {
