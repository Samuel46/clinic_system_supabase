...
import { deleteSaleAction } from "@actions/sales.action";
import { SaleDelete } from "../dialog";
import { SaleColumns } from "@type/index";

export default function SaleAction({ row }: { row: Row<SaleColumns> }) {
  ...
  const handleDelete = async () => {
    startTransition(() => {
      deleteSaleAction(row.original.id).then((result) => {
        if (result.success) {
