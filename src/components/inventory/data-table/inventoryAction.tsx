...
import { deleteInventoryAction } from "@actions/inventory.action";
import InventoryDelete from "../dialog/InventoryDelete";

export default function InventoryAction({ row }: { row: Row<InventoryColumns> }) {
  ...
  const handleEdit = async () => {
    push(`inventory/${row.original.id}`);
  };
  ...
  const handleDelete = async () => {
    startTransition(() => {
      deleteInventoryAction(row.original.id).then((result) => {
        if (result.success) {
