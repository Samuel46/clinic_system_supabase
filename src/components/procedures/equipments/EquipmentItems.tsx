import { deleteEquipmentAction } from "@actions/procedures.action";
import { cn } from "@lib/utils";
import { Component, ProcedureEquipment } from "@prisma/client";
...
const EquipmentItems: React.FC<Props> = ({ equipments, supplies }) => {
  ...
  const handleDelete = async (index: number) => {
    if (equipments[index]) {
      startTransition(() => {
        deleteEquipmentAction(equipments[index].id).then((result) => {
          if (result.success) {
