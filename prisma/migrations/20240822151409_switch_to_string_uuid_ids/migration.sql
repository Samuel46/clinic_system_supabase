/*
  ...
  - The `appointmentId` column on the `MedicalRecord` table would be dropped and recreated. This will lead to data loss if there is data in the column.
  - The `checkupId` column on the `MedicalRecord` table would be dropped and recreated. This will lead to data loss if there is data in the column.
  ...
  - Changed the type of `id` on the `Billing` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
  ...
  - Changed the type of `id` on the `Configuration` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
  ...
  - Changed the type of `id` on the `InventoryLog` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
  ...
  - Changed the type of `id` on the `MedicalCheckup` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
  ...
  - Changed the type of `id` on the `MedicalRecord` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
  ...
  - Changed the type of `id` on the `Medication` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
  ...
  - Changed the type of `id` on the `Prescription` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
  ...
  - Changed the type of `id` on the `Receipt` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
  ...
  - Changed the type of `id` on the `SaleItem` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
...
ADD COLUMN     "medicationId" UUID NOT NULL,
DROP COLUMN "tenantId",
ADD COLUMN     "tenantId" UUID NOT NULL,
...
ADD COLUMN     "inventoryId" UUID NOT NULL,
...
ADD COLUMN     "medicalRecordId" UUID,
